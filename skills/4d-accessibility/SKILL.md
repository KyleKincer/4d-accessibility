...
Trace state ownership through `JSON Stringify(Form)`, entity `fromObject` and copies into `Storage`. Keep existing root bindings intact. Entity, class-instance and 4D shared-object roots use the window registry and `options.onError`; they need no added attributes. Plain local data retains compatibility `axb*` properties containing Formulas, so exclude those properties from persistence. Follow [root ownership and failures](references/INTEGRATION.md#preserve-the-form-data-and-report-failures). Generated wrappers and explicit child registration still need private plain data; automatic children have separate state ownership.
