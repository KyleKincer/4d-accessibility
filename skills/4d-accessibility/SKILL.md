...
Use `inventory_forms.py --project-dir /path/to/Project` from the checkout or complete release kit. Search shared openers, `OBJECT SET SUBFORM`, asynchronous loaders, `CALL FORM`, `CALL WORKER`, `EXECUTE METHOD IN SUBFORM`, nonblocking `DIALOG`, `FORM GOTO PAGE`, AreaList entry/sort callbacks and `DontSortArrays` setters. Inspect runtime pages and generated controls too.

Trace state ownership through `JSON Stringify(Form)`, entity `fromObject` and copies into `Storage`. Bridge roots and registered/generated instances add `axb*` properties containing Formulas. If the root passes an entity or 4D shared object directly, leave its binding intact and treat root-state ownership as unsupported before adding start, stop or `Form.axbError` assignments. Each can raise an error on that binding. Reuse an existing separate UI-state object, or extend ownership in the bridge. Plain data objects shared between automatic children are supported and are a different case.
...
| Native array/collection/entity grids | [Native grids](references/INTEGRATION.md#add-a-native-array-list-box-without-replacing-discovery), including stable keys, selection, column descriptions and native cell controls. Use exact native bindings and preserve validation and selection handlers. |
| AreaList Pro, including repeated child grids | [AreaList configuration and assembled invoice-style example](references/INTEGRATION.md#add-an-arealist-grid-to-the-same-form). Each instance needs its own area, arrays, key pointer and scope. Check the live vendor column mapping. Use the full grid adapter for new work; legacy row summaries do not provide complete navigation. |
...
For record-changing screens, scope identifies the current record; stable row keys survive sorting and filtering. For unsaved rows, use a stable application-owned temporary identity for that lifetime. Readiness must prove that the displayed data belongs to the captured record, including late responses. Child formulas run with that child's `Form`, so its loader must update the corresponding child state. Map `onSelection` only to an existing selection-refresh controller.

Before configuring AreaList, check its actual layout, row-selection mode, array sorting and bound Text/Integer/LongInt key column. Preserve existing vendor semantics. If a compatible key column is missing or the layout is unsupported, use [the AreaList preflight](references/INTEGRATION.md#add-an-arealist-grid-to-the-same-form); do not change sorting or renumber columns just to make the adapter accept it. Repeated instances sharing all process arrays cannot represent independent data without an ownership change. Explain that change before treating it as a small hook.
