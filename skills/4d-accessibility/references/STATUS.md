...
The 0.19.4 native provider passes 120 checks across eight synthetic AppKit VoiceOver cases for delayed values, plus the 50,000-row navigation suite. An earlier compiled 4D run passed 38 checks. **The final downloaded-kit recheck fails automatic speech at a distant checkbox after native popup interaction.** AX exposes the correct checkbox state, but VoiceOver can remain at Loading. Moving to an adjacent cell and back reads the loaded checkbox, and the remaining popup/editor navigation passes. The synthetic successes do not resolve this real 4D failure. This remains a release limitation, and the regression continues to fail until corrected. [Historical passes and current failure](https://github.com/KyleKincer/4d-accessibility/blob/main/validation/grid-value-speech.json).
