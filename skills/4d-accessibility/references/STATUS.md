...
The first staged editor action now enables fast polling before the worker sleeps. Previously, a costly form could retain its one-second idle interval after entering the first character. An application's timed search could then submit that partial value. A real 4D reproduction submits `1` for an AX request of `1050` before the fix and submits `1050` afterward, in interpreted and compiled ARM execution. The ordinary and generated form suites also pass 276 checks, including keystroke filters, focus changes, validation and Undo. No application hook, timer change, native binary or component change is needed. [Before/after evidence](https://github.com/KyleKincer/4d-accessibility/blob/main/validation/initial-edit-polling.json). This correction is on the development branch; the 0.19.5 release packages predate it.
