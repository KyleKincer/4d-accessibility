...
The 0.19.4 native provider passes 120 checks across eight actual VoiceOver cases for delayed grid values. VoiceOver reads the arriving value at its current cell, including newly loaded checkbox and popup controls. Moving between cold cells, leaving the grid, unrelated AX inspection and reloading identical values preserve the reading position. The original provider fails with the same final driver and fixture. No host calls, keyboard focus changes or explicit speech requests were added. The 50,000-row native navigation suite also passes. A compiled 4D 20.8 regression passes 38 checks, including a distant checkbox that previously stayed at Loading after a native popup interaction. The notification identifies the stable table cell, so replacing its content child does not lose the update. [Before/after reports and exact hashes](https://github.com/KyleKincer/4d-accessibility/blob/main/validation/grid-value-speech.json). These native tests do not establish every 4D workflow or macOS version.
