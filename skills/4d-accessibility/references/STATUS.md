...
The 0.19.5 native provider requests one low-priority announcement when the most recently requested loading checkbox receives its value in the active window. The previous provider still failed automatic distant-cell speech in a clean compiled-4D run (38 passing checks, one failure). The corrected source passed four complete compiled runs (39 checks each), including popup use and automatic distant-checkbox speech; a fifth run against the rebuilt package is recorded in [validation](https://github.com/KyleKincer/4d-accessibility/blob/main/validation/grid-value-speech-0.19.5.json). The synthetic AppKit matrix passes 135 checks across nine VoiceOver cases, including an unrelated AX inspection and leaving a delayed checkbox before it loads. This resolves that specific regression, not the remaining control-family or distribution gates.
