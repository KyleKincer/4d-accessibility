...
## Signed release candidate, September 25

The exact published [v0.19.5-rc.5](https://github.com/KyleKincer/4d-accessibility/releases/tag/v0.19.5-rc.5) ZIP matches its published SHA-256 checksums. Its plugin bundle and component library pass strict code-signature verification. Gatekeeper accepts the stapled DMG as Notarized Developer ID, and `stapler validate` passes. The exact signed plugin and component compile in an isolated native ARM 4D 20.8 fixture and pass all 39 real VoiceOver checks, including automatic speech for the distant checkbox after native popup use. [Hashes, check names and scope](https://github.com/KyleKincer/4d-accessibility/blob/main/validation/signed-release-0.19.5.json). This does not establish every form or remote-client delivery.
...
An earlier compiled 4D 20.8 run passed 38 checks, including a loaded distant checkbox after a native popup interaction. **The downloaded CI kit later failed that same automatic-speech check.** AX still exposes the checkbox and its correct value; the VoiceOver caption stays at Loading or its navigation hint. Stable-cell layout notifications improve the synthetic cases but do not reliably solve this host workflow. The report retains both results. Removing the placeholder child, changing layout notification ordering and exposing the standard busy attribute did not fix the live failure; those experiments were reverted.

`test_grid_controls_fixture.py --run --compiled --voiceover` reproduces the case after preparing the collection/repeated/widget fixture. The driver records the failed automatic-speech assertion, tests navigation back to the loaded cell and continues the remaining popup/editor checks. Any failed assertion still makes the report false and the process exit nonzero.
